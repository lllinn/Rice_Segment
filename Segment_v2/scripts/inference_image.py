...
class OutputDataset(BaseDataset):
    def __init__(self, images_dir, output_dir, classes=None, augmentation=None, load_on_ram=False):
...
def visualize_errors(pred_masks, true_masks):
    error_masks = torch.zeros_like(true_masks).to(true_masks.device)

    # 生成错误掩码：
    # - 1表示正确预测（真实1且预测1）
    # - 2表示真实1但预测为2（rice_normal）
    # - 3表示真实1但预测为3（rice_severe）
    error_masks[(true_masks == 5)] = 1
    error_masks[(true_masks == 5) & (pred_masks == 3)] = 2
    error_masks[(true_masks == 5) & (pred_masks == 4)] = 3
...
def main(config):
    ...
    with torch.no_grad():
        for data in tqdm(output_dataloader, desc="Create all label..."):
            ...
            # outputs = visualize_errors(outputs, true_masks)
