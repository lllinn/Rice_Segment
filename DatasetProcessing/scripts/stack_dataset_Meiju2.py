...
def normalize_dataset(stack_folder, max_value = [], min_value = []):
    ...
    for file in tqdm(os.listdir(stack_folder), ncols=100):
...
if __name__ == '__main__':
    ...
    features_output_dir = r'D:/Rice2024/Meiju2/Datasets/Stack_Norm_RGB-CIs -VIs-CHM' # 特征提取输出文件夹通道
    ...
    # 标签存放位置
    label_folder = r"E:/Rice2024/Meiju2/Labels-shp" # TODO: 还没确定好
    ...
    repetition_rate = 0.1
    threshold = 0.9
    skip_log = "logs/Meiju2_All_55.json" # 跳过文件路径, 用于裁剪后 适当跳过黑色背景的图片
    normalize_log = "logs/Meiju2_All_55_Normalized.json" # 归一化文件路径，用于图像归一化，记录参数文件
    normalize_excel_output_path = "logs/Meiju2_All_55_Normalized.xlsx" # 归一化文件路径，用于图像归一化，记录参数文件
